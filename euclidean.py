...
def compute_edges_batch(batch_indices, features, threshold):
    ...
    dot = batch_feat @ features.T
    dists = np.sqrt(np.clip(2 - 2 * dot, 0, None))
    src_rows, dst_cols = np.where(dists <= threshold)

    edges = []
    for row, dst in zip(src_rows, dst_cols):
        src = batch_indices[row]
        if dst > src:
            edges.append((src, int(dst), 1 - float(dists[row, dst]) / 2))
    return edges
...
def main(args):
    ...
    existing_graphs = list(graph_dir.glob(f"euclidean_t{args.threshold}_*.pkl"))
    existing_edges = list(graph_dir.glob(f"euclidean_t{args.threshold}_*_edges.npz"))
    ...
    if existing_graphs:
        ...
        if len(edges) > 10_000_000:
            ...
            edges_path = graph_dir / f"euclidean_t{args.threshold}_{graph_time:.2f}_{graph_peak / 10**6:.2f}_edges.npz"
            np.savez_compressed(edges_path, edges=edges)
        else:
            graph_path = graph_dir / f"euclidean_t{args.threshold}_{graph_time:.2f}_{graph_peak / 10**6:.2f}.pkl"
    ...
    partition_path = graph_dir.parent / f"euclidean_t{args.threshold}_r{resolution:.4f}_{total_time:.2f}_{peak / 10**6:.2f}.txt"
