...
def compute_edges(features, k, threshold):
    ...
    valid_mask = (targets != -1) & (weights >= 1 - threshold)
...
def main(args):
    ...
    if existing_graphs:
        ...
        graph_path = graph_dir / f"knn_cosine_k{args.k}_t{args.threshold}_{graph_time:.2f}_{graph_peak / 10**6:.2f}.pkl"
    ...
    partition_path = graph_dir.parent / f"knn_cosine_k{args.k}_t{args.threshold}_r{resolution:.4f}_{total_time:.2f}_{peak / 10**6:.2f}.txt"
